...
The reason to ask for a situation is almost always to see it against the others, so that has its own tab: `シチュエーション比較`. Tick as many situations as you want, optionally give one `共通の主題` they all share - `弓を持った銀髪のエルフ` - and press `まとめて生成`. Every situation is run with the same subject and nothing else different, and each gets its own named box. Choose `タグ` or `自然文` for the output; the prose template only appears when prose is what you asked for. There is no image and no router here, which is why it is a tab rather than another mode of the workbench.

The runs are sequential, one model call per situation, so the progress line names the situation it is on rather than counting seconds. A situation that fails keeps its box and shows the reason there; the others still finish.
