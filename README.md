...
`出力の見せかた` is what makes it a comparison rather than a list. The subject is deliberately identical in every run, so the answers share most of their lines and the few that differ are buried among them. `違いだけ` says the shared part once, in `全シチュエーション共通`, and leaves each box holding only what that situation added; `全文` puts the whole pasteable prompt back. Switching between them re-reads the answers already in hand and asks the models for nothing. Tags compare tag by tag, prose line by line - half a sentence is not something anyone can paste. A run that failed is left out of the shared part, or one failure would empty it and send every box back to being the whole prompt.
