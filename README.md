...
`統合プロンプト` is joined to a stated format so it can be taken apart again: one blank line between situations, and `# <name>` opening each. A prompt's own lines are single newlines apart, so a blank line cannot occur inside one and is unambiguous as the boundary - split on blank lines, drop each block's first line, and you have the prompts back. `split_situation_blocks` in `webui.py` does exactly that, and the tests hold it against the function that does the joining rather than assuming the two agree.
...
`出力の見せかた` is an aid for reading the set, not what the tab is for, so it starts on `全文` - every block a complete pasteable prompt. The subject is deliberately identical in every run, so the answers share most of their lines and the few that differ are easy to miss among the ones that do not; `違いだけ` takes the shared lines out of the blocks, leaving each with only what its situation added, to be read alongside `共通プロンプト`. Switching between the two re-reads the answers already in hand and asks the models for nothing. Tags compare tag by tag, prose line by line - half a sentence is not something anyone can paste. A run that failed is left out of the shared part, or one failure would empty it and send every block back to being the whole prompt.
