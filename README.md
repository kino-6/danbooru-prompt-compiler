...
A situation carries a pool of candidate tags, and a run is offered five of them drawn at random rather than the lot. Handed over whole they came straight back: five plausible tags read as an answer, and copying an answer is the easiest thing a small model can do, so the same nine words arrived in the same order however many times the same situation was asked for. Tag generation from a description also ran at temperature zero, which made that identical answer certain. Measured on 戦闘 with one subject, five runs used to give one distinct result with every tag fixed; they now give five, with twenty-three of the twenty-seven tags varying and only the subject constant. The pools were widened to a minimum of eight candidates each - 516 tags across the 46 situations - because sampling five of five is not sampling.
...
Situations are plain YAML in `situations/`; drop a file in with `label`, `category`, `guidance`, optional `tags` and `order`, and it appears on the next launch - in the dropdown, in its category's group on the `シチュエーション一括生成` tab, and in the random pick. Nothing in the page is a list of situations, so adding one never means editing it: a new `category` creates its own group and lands where its `order` puts it. The tags are checked against the tag dictionary by the test suite, because a tag the dictionary has never heard of is dropped downstream without a word. They are a different axis from `presets/`, which says how an image should be rendered rather than what is happening in it.

### Generating one prompt per situation

Asking for several situations at once has its own tab: `シチュエーション一括生成`. It generates - one prompt per situation picked, from one shared subject, ready to paste.

`おまかせ生成` is the hands-off path: it invents a subject, picks that many situations at random, and runs, with nothing typed or ticked first. Choosing a subject and a handful of situations by hand is the part of a sweep that is work rather than result. The subject is composed locally from `subjects/vocabulary.yaml` - five slots, some three hundred thousand combinations - rather than asked of a model: no call to wait for, none to fail, and real variety instead of whatever sentence a small model reaches for first. It comes out in whichever shape the run wants, `1girl, solo, vampire, grey_hair, coat, wand` or "a woman vampire, with long silver hair, and blue eyes, in a long coat, carrying a small wand"; the sentence names the gender the tags do, because left unsaid the prose model chose for itself. Deciding and running happen in one call: chained as two events the second press ran on the values the first had left behind and finished in a second with the previous answer still on screen. The picks are written back into the controls, so a random run you liked can be adjusted and run again. The situations appear as one checkbox group per category rather than as one list, so all 46 fit on screen without scrolling and stay readable as more are added. Tick as many as you want - `すべて選択` and `選択解除` take the lot - optionally give one `共通の主題` they all share (`弓を持った銀髪のエルフ`), and press `まとめて生成`. Every situation is run with the same subject and nothing else different, and the answers come back as two texts rather than as a box each: `共通プロンプト`, what all of them said, and `統合プロンプト`, all of them at once in the order the categories are in rather than the order you ticked them. A box per situation meant hunting through up to forty-six of them for the one wanted. Choose `タグ` or `自然文` for the output; the prose template only appears when prose is what you asked for. There is no image and no router here, which is why it is a tab rather than another mode of the workbench.

`統合プロンプト` leads with `# 共通` and then numbers the situations - `# 1 身支度`, `# 2 儀式・式典` - each carrying only what is its own. Written whole instead, the text repeated `1girl, solo`, `looking_at_viewer` and `indoors` in every block; a set of prompts wants the base once and then the parts. The number says which prompt of the set a block is, the name says which one to reach for.

The joining is a stated format so it can be taken apart again: one blank line between blocks, `# <number> <name>` opening each. A prompt's own lines are single newlines apart, so a blank line cannot occur inside one and is unambiguous as the boundary - split on blank lines, drop each block's first line, and you have the parts back. `split_situation_blocks` in `webui.py` does exactly that, and the tests hold it against the function that does the joining rather than assuming the two agree.

The answers sit beside the controls rather than under them: stacked, they began below the fold however few there were, so every run ended with a scroll to find out what it had done. The picker keeps a full-width row of its own underneath, which is what holds it to five short rows - moved into a half-width column it doubled in height and pushed the run button off the screen instead.

The runs are sequential, one model call per situation, so ticking all 46 costs 46 calls - the progress line names the situation it is on, says what that run is doing, and counts the seconds, and `停止` is beside the run button. Each run's own stages come through it: reported only between situations the line read `（1/8） - 0.0%` for as long as the first one took, and with another program holding the card that is up to two minutes of the GPU wait before a single token is asked for. It looked hung. The wait now says it is waiting, and repeats every three seconds so the clock moves even when the stage does not. It is drawn on the status line under the buttons, in the column the buttons are in - moved across to the answers it sat 308px away from and above the button that starts the run, so pressing おまかせ生成 changed nothing anywhere the presser was looking, which reads as a button that does nothing. Before that it was left to Gradio's own choice, which rendered it on the output boxes, which are empty and hidden until a run fills them, so it landed 43px below the fold on a 1100px viewport and the first run on a page appeared to report nothing at all while every later one looked fine. A situation that fails keeps its block and gives the reason there; the others still finish.

`出力の見せかた` chooses between that shape and `ブロックごとに完結`, where every block is a complete prompt on its own and nothing is lifted out. Switching between the two re-reads the answers already in hand and asks the models for nothing. Tags are deduplicated tag by tag, prose line by line - half a sentence is not something anyone can paste. When nothing at all is shared, no `# 共通` block is written: it would be a heading over nothing. A run that failed is left out of the shared part, or one failure would empty it and send every block back to being the whole prompt. `共通プロンプト` holds the same lines as the leading block, for lifting the base out on its own.

Two things had to change before the answers were worth comparing. A situation used to be forbidden from touching anything but the pose, which is what kept an elf with a bow from being replaced by a battle - but run across several situations it produced one picture with a different verb: the same cloak, the same key light from the left, the same figure centered in frame every time. What has to survive is who the character is, so that is what is now protected, and the place, the light, the framing and the clothes the moment calls for follow the situation. The prose template also defaulted to `キャラクター設定シート`, whose `Lighting` and `Layout` sections ask for key light direction and camera distance - specifications with no bearing on what is happening, filled in identically every time. The tab defaults to `シーンイラスト` instead, which asks about place, time of day and mood. With both changed, the same elf comes back in a kitchen with steam and window light, in a dusk forest clearing under a coming storm, and in a golden afternoon clearing with a drink in hand.

The direction itself is kept out of the answer. Asked for a resting elf, the prose model wrote the direction back verbatim - `off duty and unguarded, sitting or lying down, weight let go, gear set aside, a drink or a book to hand` - which is the instruction, not a picture. The request now says the direction is something to realise rather than to repeat, and the wording is removed from the answer afterwards as well, because asking worked on most runs and not on all of them. The reference tags are held back from the prose side entirely: the tag compiler weighs them against the dictionary, and a prose model just writes them down.
...
- `situation_sweep.py`: one subject through several situations, and the merged text that holds the answers - joined to a stated format and split back out of it.
- `subject_seed.py`: a random subject composed locally from `subjects/vocabulary.yaml`, as tags or as a sentence.
