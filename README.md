...
The runs are sequential, one model call per situation, so ticking all 46 costs 46 calls - the progress line names the situation it is on rather than counting seconds, and `停止` is beside the run button. A situation that fails keeps its box and shows the reason there; the others still finish.
