...
Situations are plain YAML in `situations/`; drop a file in with `label`, `category`, `guidance`, optional `tags` and `order`, and it appears on the next launch - in the dropdown, in its category's group on the comparison tab, and with an output box of its own. Nothing in the page is a list of situations, so adding one never means editing it: a new `category` creates its own group and lands where its `order` puts it. The tags are checked against the tag dictionary by the test suite, because a tag the dictionary has never heard of is dropped downstream without a word. They are a different axis from `presets/`, which says how an image should be rendered rather than what is happening in it.
