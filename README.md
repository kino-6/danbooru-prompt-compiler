...
Asking for several situations at once has its own tab: `シチュエーション一括生成`. It generates - one prompt per situation picked, from one shared subject, ready to paste. The situations appear as one checkbox group per category rather than as one list, so all 46 fit on screen without scrolling and stay readable as more are added. Tick as many as you want - `すべて選択` and `選択解除` take the lot - optionally give one `共通の主題` they all share (`弓を持った銀髪のエルフ`), and press `まとめて生成`. Every situation is run with the same subject and nothing else different, and each gets its own named box, in the order the categories are in rather than the order you ticked them. Choose `タグ` or `自然文` for the output; the prose template only appears when prose is what you asked for. There is no image and no router here, which is why it is a tab rather than another mode of the workbench.

The runs are sequential, one model call per situation, so ticking all 46 costs 46 calls - the progress line names the situation it is on rather than counting seconds, and `停止` is beside the run button. It is drawn on the status line directly under that button: left to Gradio's own choice it rendered on the output boxes, which are all hidden until a run fills them, so it landed 43px below the fold on a 1100px viewport and the first run on a page appeared to report nothing at all while every later one looked fine. A situation that fails keeps its box and shows the reason there; the others still finish.

`出力の見せかた` is an aid for reading the set, not what the tab is for, so it starts on `全文` - the whole pasteable prompt in every box. The subject is deliberately identical in every run, so the answers share most of their lines and the few that differ are easy to miss among the ones that do not; `違いだけ` says the shared part once, in `全シチュエーション共通`, and leaves each box holding only what that situation added. Switching between the two re-reads the answers already in hand and asks the models for nothing. Tags compare tag by tag, prose line by line - half a sentence is not something anyone can paste. A run that failed is left out of the shared part, or one failure would empty it and send every box back to being the whole prompt.
